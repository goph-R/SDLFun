 ...
 * SDL_GetVideoInfo. In windowed mode the final-safety clamp below
 * (configClamp) leaves 0 alone, and main.cpp's post-resolve clamp
 * promotes it to CONFIG_W_MIN / CONFIG_H_MIN.
 */
...
{
    /* SDLFun flag style — kept for backward compatibility with existing
       shortcuts / batch files: -w N, -h N, -fullscreen, -windowed. */
