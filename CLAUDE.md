...
`local musicOn = optGet("music_on")`. Model IDs are referenced by string
from `.ent` level files (`mesh=office_desk`) and must match the snake_case
`assets.lua` key. C bindings expose a camelCase name
(`lua_register(L, "drawText", scr_draw_text)`) while the C function keeps its
`scr_` name — rename both sides together. `bullet3-*` under `vendor/` is left
as-is. Full rule lives in `../SOOB-Core/CLAUDE.md`.
