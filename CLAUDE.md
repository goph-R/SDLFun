...
- `ui.h` — 2D/HUD primitives. Builds an 8x8 ASCII bitmap font atlas in-memory at `uiInit()` (from embedded public-domain font8x8 data — no disk asset). `uiBegin`/`uiEnd` switch to ortho pixel-space, `uiText` / `uiQuad` / `uiBar` / `uiIcon` draw into it. Used for the on-screen HUD; menu system to be added later on top of the same primitives.
